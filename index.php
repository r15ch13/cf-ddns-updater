...
function updateOrCreate(CloudflareDNS $dns, string $zoneID, $record, string $type, string $name, string $content, int $ttl = 120)
{
  if (!empty($record)) {
    return $dns->updateRecordDetails($zoneID, $record->id, ['type' => $type, 'name' => $name, 'content' => $content, 'ttl' => $ttl]);
  } else {
    return $dns->addRecord($zoneID, $type, $name, $content, $ttl, false);
  }
}
...
if (empty($email) || empty($key) || empty($zone) || empty($domain)) {
  ...
  echo "ttl = Update TTL value (default 120 sec)\n\n";
...
$result = updateOrCreate($dns, $zoneID, $record, 'A', $domain, $ipv4, $ttl);
...
if (!is_null($request->get('wildcard'))) {
  ...
  $result = updateOrCreate($dns, $zoneID, $wildcard, 'A', '*.' . $domain, $ipv4, $ttl);
...
if ($ipv6) {
  ...
  $result = updateOrCreate($dns, $zoneID, $record, 'AAAA', $domain, $ipv6, $ttl);
  echo "Updated $domain to $ipv6" . PHP_EOL;
  if(!$result) { print_r($result); }

  if (!is_null($request->get('wildcard'))) {
    $wildcard = reset($dns->listRecords($zoneID, 'AAAA', '*.' . $domain)->result);
    $result = updateOrCreate($dns, $zoneID, $wildcard, 'AAAA', '*.' . $domain, $ipv6, $ttl);
